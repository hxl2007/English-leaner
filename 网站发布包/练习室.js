import {打开存档,读取,更新,总金币,等待云端同步,获取同步状态,同步存档} from './数据.js';
...
async function 刷新管理员(){if(管理员加载中)return;管理员加载中=true;管理员错误='';try{管理员用户=await 获取所有用户();}catch(e){管理员错误=e.message;}finally{管理员已加载=true;管理员加载中=false;if(路由()[0]==='设置')await 渲染();}}
...
function 练习页(记录){
  ...
  // 文章标记工具栏
  const 标记工具=p&&!记录?`<div class="文章标记工具"><button class="图标按钮小" data-action="标记文本" data-type="重点" title="标记重点">${图标('highlighter')}重点</button><button class="图标按钮小" data-action="标记文本" data-type="疑问" title="标记疑问">${图标('help-circle')}疑问</button><button class="图标按钮小" data-action="清除标记" title="清除本文标记">${图标('eraser')}清除</button><span class="标记说明">选中文字后标记，随草稿保存</span></div>`:'';
  ...
  const 题目内容=p&&q.题型!=='翻译'?`${文}<article class="阅读答题窗" aria-label="文章答题面板">${题目头}<div class="答题窗滚动区"><p class="题干" lang="en">${转义(q.题干)}</p>${作答区}${解析区}</div>${操作区}</article>`:`<article class="试题">${题目头}<p class="题干" lang="en">${转义(q.题干)}</p>${作答区}${解析区}${操作区}</article>`;
...
async function 提交检查(){
  ...
  确认('提交并生成成绩？',`已完成 ${c.题目编号.length} 道题。提交后按得分比例获得金币，最高 ${奖励规则[c.模块]} 金币。`,'提交确认','提交并评分');
}
...
async function 处理点击(b){if(!b||b.disabled)return;const 行=b.dataset.action;try{
  ...
  if(行==='定位空'){const p=索引.题目.get(id)?.篇章;当前题=c.单元.findIndex(u=>u.篇章===p&&索引.题目.get(u.题目)?.原题号===Number(b.dataset.blank));面板收起=false;if(!记录)await 修改草稿(c.编号,d=>({...d,当前题}));await 渲染();return;}
...
async function 执行导入(){if(!待导入||忙)return;忙=true;try{await 载入答案();const 导入数据=待导入;const 记录=导入数据.记录.map(r=>{const c=验证练习(r,题库);if(!Number.isFinite(c.提交时间)||!Number.isFinite(c.用时)||c.用时<0)throw new Error('存档时间无效。');return {...c,...通用评分(c,答案)};});let 草稿=导入数据.草稿?验证练习(导入数据.草稿,题库):null;if(草稿)草稿={...草稿,编号:新编号()};const 合法=new Set([...题库.题目.filter(q=>q.题源==='原创').map(q=>q.编号),...题库.篇章.filter(p=>p.题源==='原创').map(p=>p.编号)]),已抽=(导入数据.已抽原创||[]).filter(id=>合法.has(id));for(const c of [...记录,...(草稿?[草稿]:[])])if(c.题源==='原创')已抽.push(...c.题目编号,...c.篇章编号);存档=await 更新(s=>{const 合并记录=[...new Map([...s.记录,...记录].map(r=>[r.编号,r])).values()];const 记录金币=合并记录.reduce((n,r)=>n+Number(r.金币||0),0);return {...s,版本:3,记录:合并记录.sort((a,b)=>a.提交时间-b.提交时间),收藏:[...new Set([...s.收藏,...(导入数据.收藏||[]).filter(id=>索引.题目.has(id))])],草稿:草稿||s.草稿,已抽原创:[...new Set([...(s.已抽原创||[]),...已抽])],消费金币:Math.min(记录金币,Math.max(0,Number(s.消费金币)||0,Number(导入数据.消费金币)||0)),背包:Object.fromEntries([...new Set([...Object.keys(s.背包||{}),...Object.keys(导入数据.背包||{})])].map(id=>[id,Math.max(0,Number(s.背包?.[id])||0,Number(导入数据.背包?.[id])||0)])),小段:Math.max(Number(s.小段)||1,Number(导入数据.小段)||1)};});待导入=null;弹窗.close();const 已同步=await 等待云端同步();sessionStorage.setItem('升本练习室导入提示',已同步?'导入成功，已同步云端，页面已刷新。':'导入成功，已保存到本机，云端待同步。页面已刷新。');location.reload();}finally{忙=false;}}
