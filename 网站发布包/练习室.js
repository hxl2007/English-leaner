import {打开存档,读取,更新,总金币,等待云端同步} from './数据.js';
import {模块列表,奖励规则,修为等级,修为信息,兑换物品,生成练习,检查库存,可抽内容,建立题库索引,补齐旧练习,通用评分,题目分值,未答题,验证练习,新编号} from './组卷.js';
import {初始化认证,是否已登录,获取当前用户,退出登录,是管理员,获取所有用户,更新用户状态,管理员手机号} from './认证.js';
...
function 当前小段(){return Math.max(1,Math.min(9,Number(存档.小段)||1));}
function 当前战力(){const 修为=修为信息(总金币(存档)),境界战力=修为等级.findIndex(x=>x.名称===修为.当前.名称)*100;const 物品战力=Object.entries(存档.背包||{}).reduce((n,[id,数量])=>n+(兑换物品.find(x=>x.编号===id)?.战力||0)*Number(数量||0),0);return 境界战力+(当前小段()-1)*10+物品战力;}
...
function 练习页(记录){
  ...
  const 题目头=`<div class="题目头"><strong>第 ${String(当前题+1).padStart(2,'0')} 题 / ${总}</strong><span>${q.题型||'单选'} · ${数值(题目分值(c,当前题))} 分</span></div>`;
  ...
  const 操作区=`<div class="题目操作"><button class="按钮 次要" data-action="翻题" data-step="-1" ${当前题===0?'disabled':''}>${图标('chevron-left')}上一题</button><div class="按钮组"><button class="图标按钮" data-action="收藏" aria-label="${存档.收藏.includes(q.编号)?'取消收藏':'收藏本题'}" title="收藏本题" aria-pressed="${存档.收藏.includes(q.编号)}">${图标('bookmark')}</button>${!记录?`<button class="图标按钮" data-action="标记" aria-label="标记稍后检查" title="标记稍后检查" aria-pressed="${(c.标记||[]).includes(q.编号)}">${图标('flag')}</button>`:''}</div><button class="按钮" data-action="${当前题===总-1&&!记录?'提交检查':'翻题'}" data-step="1" ${当前题===总-1&&记录?'disabled':''}>${当前题===总-1&&!记录?'提交':'下一题'}${图标('chevron-right')}</button></div>`;
  const 题目内容=p&&q.题型!=='翻译'?`${文}<article class="阅读答题窗">${题目头}<p class="题干" lang="en">${转义(q.题干)}</p>${作答区}${解析区}${操作区}</article>`:`<article class="试题">${题目头}<p class="题干" lang="en">${转义(q.题干)}</p>${作答区}${解析区}${操作区}</article>`;
...
function 修为页(){const 金币=总金币(存档),修为=修为信息(金币);return `${页头('修为与战力','累计金币决定大境界，丹药可以提升当前小段，装备与功法会增加战力。',`<a class="按钮 次要" href="#兑换">${图标('gift')}金币兑换</a>`)}<section class="境界概况"><div>${图标('award')}<h2>${修为.当前.名称} · ${当前小段()}段</h2><p>累计获得 ${金币} 金币 · 可用 ${可用金币()} 金币</p></div><div><p>当前战力：<strong>${当前战力()}</strong>${修为.下一?` · 距离${修为.下一.名称}还需 ${修为.下一.金币-金币} 金币`:''}</p><div class="进度条"><div style="width:${修为.进度}%"></div></div></div></section><div class="分区头"><h2>修为境界</h2></div><ol class="修为阶梯">${修为等级.map(x=>`<li class="${金币>=x.金币?'已达':''}">${图标(金币>=x.金币?'check-circle-2':'circle')}<strong>${x.名称}</strong><span>${x.金币} 金币</span>${修为.当前===x?'<span class="徽标">当前</span>':''}</li>`).join('')}</ol><div class="分区头"><h2>完成奖励上限</h2></div><div class="奖励列表">${Object.entries(奖励规则).map(([k,n])=>`<div><span>${k==='整卷'?'全套卷子':k}</span><strong>最高 ${n} 金币</strong></div>`).join('')}</div>`;}
function 兑换页(){
  const 分组=['功法','装备','法器','丹药'];
  const 物品卡=(x,类)=>{
    const 数量=Number(存档.背包?.[x.编号]||0),拥有=数量>0;
    const 效果=x.战力?`+${x.战力} 战力`:类==='丹药'?'提升1小段':'';
    const 操作=类==='丹药'&&拥有
      ?`<button class="按钮 小 次要" data-action="服用丹药" data-id="${x.编号}">服用</button>`
      :`<button class="按钮 小" data-action="兑换物品" data-id="${x.编号}" ${可用金币()<x.价格?'disabled':''}>兑换</button>`;
    return `<article class="物品卡 ${拥有?'拥有':''}"><div class="物品卡头"><span class="徽标">${x.类别}</span><strong>${x.名称}</strong></div><p>${x.说明}</p><div class="物品卡底"><span>${效果} · ${x.价格} 金币${拥有?` · 已有 ${数量}`:''}</span>${操作}</div></article>`;
  };
  const 分区=分组.map(类=>`<section class="物品区"><div class="分区头"><h2>${类}</h2></div><div class="物品网格">${兑换物品.filter(x=>x.类别===类).map(x=>物品卡(x,类)).join('')}</div></section>`).join('');
...
function 设置页(){const 用户=获取当前用户();const 管理员区=是管理员()?`<section class="管理员面板"><div class="分区头"><div><h2>管理员账户管理</h2><p class="副标题">可查看云端注册账户并控制登录状态。</p></div><span class="徽标">管理员</span></div><div class="管理员表格包"><table class="表格 管理员表格"><thead><tr><th>用户</th><th>手机号</th><th>注册时间</th><th>最后登录</th><th>状态</th><th>操作</th></tr></thead><tbody>${管理员用户.map(x=>`<tr><td>${转义(x.名称)}</td><td>${转义(x.手机号)}</td><td>${日期(x.注册时间)}</td><td>${x.最后登录?日期(x.最后登录):'未登录'}</td><td><span class="徽标 ${x.禁用?'红':''}">${x.禁用?'已禁用':'正常'}</span></td><td>${x.手机号===管理员手机号?'<span class="副标题">管理员</span>':`<button class="按钮 小 ${x.禁用?'次要':''}" data-action="切换用户状态" data-phone="${x.手机号}" data-disabled="${x.禁用?'true':'false'}">${x.禁用?'启用':'禁用'}</button>`}</td></tr>`).join('')}</tbody></table></div></section>`:'';return `${页头('学习存档','管理答题进度、记录与收藏。')}<div class="设置行"><div><h3>登录账号</h3><p>${转义(用户?.名称||'未登录')} · ${转义(用户?.手机号||'')}</p></div><button class="按钮 次要" data-action="退出登录">${图标('log-out')}退出登录</button></div><div class="设置行"><div><h3>当前存档</h3><p>${存档.记录.length} 次练习 · ${总金币(存档)} 金币 · ${存档.收藏.length} 道收藏</p></div><span class="徽标">云端同步</span></div>${管理员区}<div class="设置行"><div><h3>备份与迁移</h3><p>进度会保存在当前账号的云端存档，换手机或电脑登录同一账号即可继续学习；导出存档可作为额外备份。</p></div><div class="按钮组"><button class="按钮 次要" data-action="导出">${图标('download')}导出存档</button><label class="按钮 次要" for="导入文件">${图标('upload')}导入存档</label><input id="导入文件" type="file" class="隐" accept="application/json,.json"></div></div><div class="设置行"><div><h3>页面风格</h3><p>选择喜欢的颜色，设置会保存在当前浏览器。</p></div><div class="主题选择" role="group" aria-label="页面风格">${主题列表.map(([id,名])=>`<button class="${主题===id?'当前':''}" data-action="主题" data-value="${id}" aria-pressed="${主题===id}"><span class="主题色 ${id}"></span>${名}</button>`).join('')}</div></div><div class="设置行"><div><h3>原创题使用记录</h3><p>原创题在开始组卷时记为已抽，单块与整卷共享未抽题库；抽完后需要补充题库。当前原创库包含 500 道单选、60 篇阅读、20 篇完形和 100 道翻译。</p></div></div><div class="设置行"><div><h3>题目校对</h3><p>题目保留照片出处，答案与解析重新编写。新增照片中的清晰翻译和完形已纳入原题库；照片中无法确认的字词会暂不抽取。</p></div></div><div class="警告">退出账号后，本机仍会保留当前账号的离线缓存；下次登录会与云端时间较新的存档合并。</div>`;}
...
async function 兑换(编号){const 物=兑换物品.find(x=>x.编号===编号);if(!物)throw new Error('物品不存在。');if(可用金币()<物.价格)throw new Error('可用金币不足。');存档=await 更新(s=>({...s,版本:3,消费金币:(Number(s.消费金币)||0)+物.价格,背包:{...(s.背包||{}),[编号]:Number(s.背包?.[编号]||0)+1}}));await 渲染();提示(`已兑换${物.名称}，战力增加${物.战力||0}。`);}
async function 服用(编号){const 物=兑换物品.find(x=>x.编号===编号);if(!物||物.类别!=='丹药')throw new Error('丹药不存在。');if(!Number存档(编号))throw new Error('背包中没有这枚丹药。');存档=await 更新(s=>({...s,版本:3,小段:Math.min(9,Math.max(1,Number(s.小段)||1)+1),背包:{...(s.背包||{}),[编号]:Number(s.背包?.[编号]||0)-1}}));await 渲染();提示(`已服用${物.名称}，当前为第${当前小段()}段。`);}
function Number存档(编号){return Number(存档.背包?.[编号]||0)>0;}
async function 提交检查(){
...
async function 处理点击(b){if(!b||b.disabled)return;const 行=b.dataset.action;try{
  ...
  if(行==='退出登录'){await 等待云端同步();退出登录();location.href='./login.html';return;}
  if(行==='切换用户状态'){
...
document.addEventListener('change',async e=>{try{if(e.target.name==='自评'){const c=存档.草稿;if(c?.阶段!=='自评')return;const id=c.题目编号[当前题],分=Number(e.target.value);写入队列=写入队列.then(()=>修改草稿(c.编号,d=>({...d,自评:{...d.自评,[id]:分}}))).then(渲染).catch(错误提示);await 写入队列;return;}if(e.target.id!=='导入文件'||!e.target.files[0])return;const f=e.target.files[0];if(f.size>40000000)throw new Error('存档文件过大。');const v=JSON.parse(await f.text()),导出数据=v.存档||v;if(v.应用&&v.应用!=='升本练习室')throw new Error('存档格式无效。');if(![1,2,3].includes(导出数据.版本)||!Array.isArray(导出数据.记录)||!Array.isArray(导出数据.收藏))throw new Error('存档格式无效。');待导入=导出数据;确认('合并学习存档？','相同记录不会重复计币；本机草稿优先保留，金币、背包和已抽题记录同时合并。','导入确认','合并存档');}catch(err){错误提示(err);}finally{if(e.target.id==='导入文件')e.target.value='';}});
async function 执行导入(){if(!待导入||忙)return;忙=true;try{await 载入答案();const 导入数据=待导入;const 记录=导入数据.记录.map(r=>{const c=验证练习(r,题库);if(!Number.isFinite(c.提交时间)||!Number.isFinite(c.用时)||c.用时<0)throw new Error('存档时间无效。');return {...c,...通用评分(c,答案)};});let 草稿=导入数据.草稿?验证练习(导入数据.草稿,题库):null;if(草稿)草稿={...草稿,编号:新编号()};const 合法=new Set([...题库.题目.filter(q=>q.题源==='原创').map(q=>q.编号),...题库.篇章.filter(p=>p.题源==='原创').map(p=>p.编号)]),已抽=(导入数据.已抽原创||[]).filter(id=>合法.has(id));for(const c of [...记录,...(草稿?[草稿]:[])])if(c.题源==='原创')已抽.push(...c.题目编号,...c.篇章编号);存档=await 更新(s=>{const 合并记录=[...new Map([...s.记录,...记录].map(r=>[r.编号,r])).values()];const 记录金币=合并记录.reduce((n,r)=>n+Number(r.金币||0),0);return {...s,版本:3,记录:合并记录.sort((a,b)=>a.提交时间-b.提交时间),收藏:[...new Set([...s.收藏,...(导入数据.收藏||[]).filter(id=>索引.题目.has(id))])],草稿:s.草稿||草稿,已抽原创:[...new Set([...(s.已抽原创||[]),...已抽])],消费金币:Math.min(记录金币,Number(s.消费金币||0)+Number(导入数据.消费金币||0)),背包:Object.fromEntries([...new Set([...Object.keys(s.背包||{}),...Object.keys(导入数据.背包||{})])].map(id=>[id,Number(s.背包?.[id]||0)+Number(导入数据.背包?.[id]||0)])),小段:Math.max(Number(s.小段)||1,Number(导入数据.小段)||1)};});待导入=null;弹窗.close();sessionStorage.setItem('升本练习室导入提示','导入成功，存档已合并，页面已刷新。');location.reload();}finally{忙=false;}}
...
window.addEventListener('云端存档更新',async()=>{if(!存档)return;try{存档=await 读取();await 渲染();提示('已同步其他设备的最新进度。');}catch(e){错误提示(e);}});
