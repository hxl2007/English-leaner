...
function 设置页() {return `${页头('学习存档','管理你的答题进度、练习记录和收藏。')}<div class="设置行"><div><h3>当前存档</h3><p>${存档.记录.length} 次练习 · ${总金币(存档)} 金币 · ${存档.收藏.length} 道收藏</p></div><span class="徽标">本机保存</span></div><div class="设置行"><div><h3>备份与迁移</h3><p>存档保存在当前浏览器。换手机或电脑时，可导出备份后在另一台设备导入。</p></div><div class="按钮组"><button class="按钮 次要" data-action="导出">${图标('download')}导出存档</button><button class="按钮 次要" data-action="导入">${图标('upload')}导入存档</button><input type="file" id="导入文件" accept=".json,application/json" class="隐"></div></div><div class="设置行"><div><h3>练习奖励</h3><p>每次答完 25 题并提交，奖励 25 金币。同一次提交只入账一次；开始新一轮练习后可以再次获得。</p></div></div><div class="设置行"><div><h3>题库说明</h3><p>48 张照片整理为 360 道题，原手写答案与批注已移除。答案和中文解析按语法语义重新编写。<br>最后一套含 10 道新题和 15 道复习题；有歧义的原题在解析中说明，合理答案均可得分。</p></div></div><div class="警告">清除浏览器网站数据会移除本机存档，建议定期导出备份。当前版本不提供账号云同步。</div>`;}
let 渲染序号=0;
async function 渲染() {
