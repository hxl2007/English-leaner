...
export const 奖励规则 = {单选:25, 阅读:40, 完形:30, 翻译:25, 整卷:100};
export const 修为等级 = [
...
export function 通用评分(练习,答案) {
  if(未答题(练习).length)throw new Error('请完成全部题目后再提交。');
  const 单题得分=[],对错=[];
  ...
  return {分数:Math.round(单题得分.reduce((n,x)=>n+x,0)*100)/100,满分:练习.模块==='整卷'?85:100,金币:奖励规则[练习.模块],单题得分,对错,含自评:练习.单元.some(q=>q.题型==='翻译')};
}
export function 验证练习(练习,题库) {
  ...
  const c=补齐旧练习(练习,题库);
  if(!['原题','原创'].includes(c.题源)||![...模块列表,'整卷'].includes(c.模块)||!Array.isArray(c.单元)||c.单元.length!==c.题目编号.length)throw new Error('存档的练习结构无效。');
  const 数={单选:0,阅读:0,完形:0,翻译:0};
  const 篇章=new Map();
  for(const [i,u] of c.单元.entries()) {
    const q=索引.题目.get(u.题目);
    if(u.题目!==c.题目编号[i]||u.题型!==(q.题型||'单选')||(q.题源||'原题')!==c.题源||u.篇章!==(q.篇章||null))throw new Error('存档题目与题库不一致。');
    数[u.题型]++;
  ...
  for(const [id,题] of 篇章){const p=索引.篇章.get(id);if(!p||p.题目编号.length!==题.size||!p.题目编号.every(q=>题.has(q)))throw new Error('存档文章或小题不完整。');}
  ...
  if(数.单选!==需求.单选||数.翻译!==需求.翻译||阅读数!==需求.阅读||完形数!==需求.完形||数.完形!==需求.完形*20)throw new Error('存档题量不符合组卷规则。');
